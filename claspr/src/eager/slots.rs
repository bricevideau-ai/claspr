...
// ── `Input`'s slot half ──────────────────────────────────────────────────
//
// The slot-binding methods of [`Input`] live HERE rather than beside the
// lend/resolve half in `eager.rs`: they are written almost entirely in this
// module's vocabulary ([`SlotBinder`], [`SlotState`], [`SlotCell`],
// [`BindMode`], `probe_lent`/`provide`), so an agent sent to "slot internals"
// by ARCHITECTURE.md finds the whole bind path in one file. `eager.rs` keeps
// the run-time half (resolve / lend / rehome), which is engine-core, not slots.
impl<T> Input<T> {
