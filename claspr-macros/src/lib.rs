...
fn classify_image_param(ty: &Type) -> Option<ImageInfo> {
    ...
    let format = if let Some(fname) = format_tok {
        // `format=<name>`. rust-gpu's `Image!` macro accepts
        // snake-case Vulkan/GLSL names (`rgba32f`, `rgba8_snorm`,
        // `r32i`, …) — translate to claspr's PascalCase
        // `cl_channel_*`-derived names. A user who wrote the
        // PascalCase name directly (e.g. `format=R32G32B32A32Float`)
        // passes through unchanged via the catch-all.
        let translated = snake_format_to_claspr(&fname);
        let ident = quote::format_ident!("{}", translated);
...
/// Translate the snake-case `format=` value rust-gpu's `Image!`
/// macro accepts (`rgba32f`, `rgba8_snorm`, `r32i`, …) into the
/// PascalCase claspr `format::*` ZST name.
///
/// claspr's runtime format namespace follows OpenCL's
/// `cl_channel_type` convention (`R8G8B8A8Uint` vs Vulkan's
/// `Rgba8ui`) — see `claspr/src/image.rs` module docs for the
/// rationale. This table is the bridge.
///
/// A name that isn't in the table passes through unchanged on the
/// assumption it's already a PascalCase claspr format. If it isn't,
/// the downstream `::claspr::image::format::<name>` reference will
/// fail to resolve at compile time with a clear "no item named"
/// error pointing at the unrecognised format — preferable to a
/// silent fallback because format mismatches between kernel and
/// host silently corrupt image writes (see the
/// `R8G8B8A8Unorm`-vs-`R8G8B8A8Uint` Intercept-Layer bug history
/// in `claspr/src/image.rs`).
///
/// Formats present in rust-gpu but absent from claspr — typically
/// because OpenCL has no `cl_channel_data_type` for them
/// (`r11f_g11f_b10f`, `rgb10_a2`, `rgb10_a2ui`, `r64ui`, `r64i`)
/// — pass through and fail at the resolution step. Add to
/// `claspr::image::format` first if you need them.
fn snake_format_to_claspr(snake: &str) -> String {
    match snake {
        // RGBA8 family
        "rgba8ui" => "R8G8B8A8Uint",
        "rgba8i" => "R8G8B8A8Sint",
        "rgba8" => "R8G8B8A8Unorm",
        "rgba8_snorm" => "R8G8B8A8Snorm",
        // RGBA16 family
        "rgba16ui" => "R16G16B16A16Uint",
        "rgba16i" => "R16G16B16A16Sint",
        "rgba16" => "R16G16B16A16Unorm",
        "rgba16_snorm" => "R16G16B16A16Snorm",
        "rgba16f" => "R16G16B16A16Float",
        // RGBA32 family
        "rgba32f" => "R32G32B32A32Float",
        "rgba32ui" => "R32G32B32A32Uint",
        "rgba32i" => "R32G32B32A32Sint",
        // RG32 family
        "rg32f" => "R32G32Float",
        "rg32ui" => "R32G32Uint",
        // R32 family
        ...
        // R8
        "r8" => "R8Unorm",
        // Already PascalCase? Pass through and let resolution
        // diagnose any typo or missing format at use site.
        other => other,
    }
    .to_string()
}
...
mod tests {
    ...
    fn parse(input: &str) -> (Option<String>, Option<String>, Option<String>) {
        let tokens: TokenStream2 = input.parse().expect("parse");
        parse_image_tokens(tokens)
    }

    #[test]
    fn parses_type_only() {
        let (access, format, type_) = parse("2D, type=u32, sampled=false");
        assert_eq!(access, None);
        assert_eq!(format, None);
        assert_eq!(type_, Some("u32".into()));
    }

    #[test]
    fn parses_format_field() {
        let (_, format, _) = parse("2D, format=rgba32f, sampled=false");
        assert_eq!(format, Some("rgba32f".into()));
    }

    #[test]
    fn parses_access_field() {
        let (access, ..) = parse("2D, access=read_only, type=f32, sampled=true");
        assert_eq!(access, Some("read_only".into()));
    }

    #[test]
    fn snake_translates_known_formats() {
        // Spot-check the bridge entries — each family represented.
        assert_eq!(snake_format_to_claspr("rgba32f"), "R32G32B32A32Float");
        assert_eq!(snake_format_to_claspr("rgba8_snorm"), "R8G8B8A8Snorm");
        assert_eq!(snake_format_to_claspr("rgba8ui"), "R8G8B8A8Uint");
        assert_eq!(snake_format_to_claspr("r32f"), "R32Float");
        assert_eq!(snake_format_to_claspr("r32i"), "R32Sint");
        assert_eq!(snake_format_to_claspr("rg32f"), "R32G32Float");
        assert_eq!(snake_format_to_claspr("r8"), "R8Unorm");
    }

    #[test]
    fn snake_passes_through_pascalcase() {
        // A user who wrote the PascalCase name directly stays
        // PascalCase — resolution succeeds against
        // `::claspr::image::format::R32G32B32A32Float`.
        assert_eq!(
            snake_format_to_claspr("R32G32B32A32Float"),
            "R32G32B32A32Float"
        );
    }

    #[test]
    fn snake_passes_through_unknown_so_resolution_errors_clearly() {
        // Formats present in rust-gpu but not in claspr's
        // `cl_channel_*`-derived list pass through; the downstream
        // `::claspr::image::format::r11f_g11f_b10f` reference fails
        // to resolve at compile time with a clear error pointing
        // at the unrecognised format — preferable to a silent
        // fallback (see Intercept-Layer Unorm-vs-Uint bug history
        // in `claspr/src/image.rs`).
        assert_eq!(
            snake_format_to_claspr("r11f_g11f_b10f"),
            "r11f_g11f_b10f"
        );
    }
}
