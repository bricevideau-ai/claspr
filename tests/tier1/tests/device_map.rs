...
fn submit_returns_pending_then_wait_yields_guard() {
    ...
    let pending = buf.map().submit(&ctx).expect("submit map");
...
fn submit_mut_pending_derefmut_after_wait() {
    ...
    let pending = buf.map_mut().submit(&ctx).expect("submit map_mut");
    ...
    let g = buf.map().wait(&ctx).expect("readback map");
    for (i, &v) in g.iter().enumerate() {
...
fn release_returns_unmap_event_for_cross_queue_chaining() {
    ...
    let mut g = buf.map_mut().wait(&aux_queue).expect("map_mut on aux");
    for (i, slot) in g.iter_mut().enumerate() {
