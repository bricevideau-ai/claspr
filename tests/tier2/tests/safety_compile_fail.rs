//! Compile-fail surface for claspr-async's Tier 2 safety-by-construction
//! story, driven by [`ui_test`].
//!
//! Each fixture in `tests/tier2/compile_fail/` deliberately attempts a
//! Tier 2 shape the type system should reject — parallel writes to one
//! buffer through `bundle!`, `Arc<DeviceSlice>` in a writable kernel
//! slot, use-after-move in a sequential chain, host-view borrow escape,
//! or use-after-release of a host view. The test passes if rustc
//! rejects every fixture.
//!
//! Green means: the Tier 2 safety invariants are enforced by types,
//! not folklore. Red means: a safety guarantee regressed — investigate
//! the failing fixture before re-blessing.
//!
//! See [[project_claspr_safety_audit_followups]] for the audit that
//! enumerated these cases.
//!
//! ## Why ui_test instead of trybuild
//!
//! Same reasons as `tests/tier1/tests/image_compile_fail.rs` — see
//! that file's module doc for the full rationale. Short version:
//! trybuild forks `cargo check` per test (5min+ per invocation in
//! this workspace) and has a known intermittent false-success bug;
//! ui_test in direct-rustc mode against pre-built rlibs runs each
//! fixture in ~0.6s with no bulk-mode hazard.
//!
...
fn main() -> Result<()> {
    ...
    let externs = ExternRlibs::discover(&["claspr", "claspr_async", "claspr_test_kernels"])?;
...
fn make_config(path: &str, mode: Mode, args: &ui_test::Args, externs: &ExternRlibs) -> Config {
    ...
    // transitive deps (opencl3, claspr-async's deps, etc.). When the
    // parent build used a `--target` triple we must replicate it so
    // rustc's lookup matches the triple stamped into the rlibs'
    // metadata, plus a secondary `-L` for host artifacts the way
    // cargo does it. Mirrors `image_compile_fail.rs`'s harness exactly.
