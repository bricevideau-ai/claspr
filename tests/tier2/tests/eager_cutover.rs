...
///
/// This is the shape that exercises a downstream **device** op
/// (`.and_then(download)`) after a *failing* host seam. The start-gate fix (the
/// whole graph is enqueued before any of it runs, so the seam's negative
/// `proceed` can't race a downstream blocking transfer's wait-commit) closes the
/// legacy NEO lost-wakeup deadlock; the worker is joined at the terminal. Solid
/// on NEO + rusticl. pocl may still SIGSEGV ~8% on the terminated-read cleanup
/// (a pocl-internal bug, tracked in NOTES); the abort SEMANTICS are correct on
/// all three. See NOTES "and_then_host error path".
#[test]
